...
import {
  ...
  Swords, Crown, Coffee, Star, X as XIcon
} from "lucide-react";
...
function distributeWithSeeding(entities, numGroups, perGroupCount, isSeed) {
  ...
  const place = (item) => {
    let attempts = 0;
    while (groups[gi % numGroups].length >= perGroupCount && attempts < numGroups) {
      gi++; attempts++;
    }
...
const Theme = () => (
  <style>{`
    ...
    .bt-chip-neutral { background: rgba(16,38,43,0.08); color: var(--ink); }
    ...
    .bt-option-card-active { border-color: var(--ocean); background: rgba(20,124,134,0.08); }
...
function AthletesTab({ athletes, setAthletes, groupsExist }) {
  ...
  const addAthlete = () => {
    ...
    if (!trimmed) return;
    setAthletes((prev) => [...prev, { id: uid(), name: trimmed, category: category || null }]);
    setName("");
  };
  ...
  const addBulk = () => {
    ...
    setAthletes((prev) => [...prev, ...names.map((n) => ({ id: uid(), name: n, category: null }))]);
    setBulk("");
  };
      ...
      <Section icon={Users} title="Cadastro de atletas" subtitle="Adicione os jogadores que vão participar do torneio.">
        ...
        <details className="mb-2">
          ...
          <div className="mt-3 flex flex-col gap-2">
            ...
            />
            <p className="text-xs text-stone-400">A categoria pode ser definida depois, individualmente, na lista abaixo.</p>
...
function DrawTab({ athletes, config, setConfig, onDraw, groups, onReset, manualPairs, setManualPairs }) {
  ...
  const leftover =
    config.mode === "fixed" && config.pairMode === "manual"
      ? Math.max(0, manualPairs.length - totalDuplasNeeded)
      : Math.max(0, athletes.length - totalNeededPlayers);
          ...
          <div className="text-sm">
            {config.mode === "fixed" && config.pairMode === "manual" ? (
              <p className="bt-text-ink">
                Necessário: <span className="font-bold">{totalDuplasNeeded}</span> duplas &nbsp;•&nbsp; Definidas:{" "}
                <span className="font-bold">{manualPairs.length}</span>
              </p>
            ) : (
              <p className="bt-text-ink">
                Necessário: <span className="font-bold">{totalNeededPlayers}</span> atletas &nbsp;•&nbsp; Cadastrados:{" "}
            ...
            {enough && leftover > 0 && (
              <p className="text-stone-500 mt-1">
                {leftover} {config.mode === "fixed" && config.pairMode === "manual" ? "dupla(s)" : "atleta(s)"} ficarão de fora (reservas).
              </p>
            )}
          </div>
...
/* ---------------------------------------------------------------
   APP
----------------------------------------------------------------*/
const STORAGE_KEY = "beach-tennis-tournament-state-v3";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await storage.set(
          STORAGE_KEY,
          JSON.stringify({ athletes, groups, manualPairs, config, classConfig, playoff })
        );
        setSaveErr(false);
      } catch (e) {
        setSaveErr(true);
  ...
  }, [athletes, groups, manualPairs, config, classConfig, playoff, loaded]);
  ...
  const handleDraw = () => {
    ...
    const totalNeededPlayers = perGroupPlayers * numGroups;
    const totalDuplasNeeded = duplasPerGroup * numGroups;
    ...
    if (mode === "fixed") {
      if (pairMode === "manual") {
        if (manualPairs.length < totalDuplasNeeded) return;
        duplasPool = shuffle(manualPairs).slice(0, totalDuplasNeeded);
      } else {
        if (athletes.length < totalNeededPlayers) return;
        const pool = shuffle(athletes.map((a) => a.id)).slice(0, totalNeededPlayers);
      ...
      if (athletes.length < totalNeededPlayers) return;
      playerPool = shuffle(athletes.map((a) => a.id)).slice(0, totalNeededPlayers);
    }

    const groupedEntities =
      mode === "fixed"
        ? distributeWithSeeding(
            duplasPool, numGroups, duplasPerGroup,
            seedCategoryA ? (d) => d.playerIds.some((id) => athletesMap[id]?.category === "A") : () => false
          )
        : distributeWithSeeding(
            playerPool, numGroups, playersPerGroup,
