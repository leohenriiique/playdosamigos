...
function computeQualifiers(athletes, groups, classConfig) {
  ...
  if (classConfig.source === "geral") {
    ...
    // saíram, o 3º colocado de cada chave também classifica — mas se os dois
    // melhores vieram da MESMA chave, essa chave sozinha fornece o 3º E o 4º
    // colocado (senão sobraria só 1 vaga extra em vez de 2).
    direct = overallRanking.slice(0, 2).map((p) => ({ ...p, position: p.groupPosition }));
    const sourceGroupNames = [...new Set(direct.map((p) => p.groupName))];
    const bonusThirds = [];
    if (sourceGroupNames.length === 1) {
      const gr = groupRankings.find(({ group }) => group.name === sourceGroupNames[0]);
      if (gr?.ranked[2]) bonusThirds.push({ ...gr.ranked[2], groupName: sourceGroupNames[0], position: 3 });
      if (gr?.ranked[3]) bonusThirds.push({ ...gr.ranked[3], groupName: sourceGroupNames[0], position: 4 });
    } else {
      sourceGroupNames.forEach((gName) => {
        const gr = groupRankings.find(({ group }) => group.name === gName);
        if (gr?.ranked[2]) bonusThirds.push({ ...gr.ranked[2], groupName: gName, position: 3 });
      });
    }
      ...
      >
        ...
        {classConfig.source === "geral" ? (
          ...
          <div className="bt-bg-sand rounded-xl p-3 text-sm text-stone-600">
            Sem campos pra configurar aqui — a regra já é fixa: os <b>2 melhores do ranking geral</b> classificam direto, e o <b>3º colocado</b> da chave (ou chaves) de onde eles vieram também avança. Se os dois melhores saírem da <b>mesma chave</b>, essa chave sozinha classifica o <b>3º e o 4º colocados</b> (pra manter 2 vagas extras de qualquer jeito).
          </div>
        ) : (
          <>
