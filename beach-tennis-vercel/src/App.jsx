...
function computeQualifiers(athletes, groups, classConfig) {
  ...
  if (classConfig.source === "geral") {
    ...
    // Os N melhores do ranking geral (classConfig.topPerGroup) classificam direto.
    // Nas chaves de onde eles saíram, a posição extra (classConfig.extraPosition)
    // também classifica — e se mais de um "direto" veio da MESMA chave, essa chave
    // sozinha fornece posições sucessivas (ex: 3º e 4º) em vez de só uma, até
    // completar classConfig.extraCount vagas no total.
    const topN = Math.max(0, classConfig.topPerGroup || 0);
    direct = overallRanking.slice(0, topN).map((p) => ({ ...p, position: p.groupPosition }));

    const extraPos = Math.max(1, classConfig.extraPosition || 1);
    const extraCount = Math.max(0, classConfig.extraCount || 0);

    const orderedGroups = [];
    const directCountByGroup = {};
    direct.forEach((p) => {
      if (!(p.groupName in directCountByGroup)) orderedGroups.push(p.groupName);
      directCountByGroup[p.groupName] = (directCountByGroup[p.groupName] || 0) + 1;
    });
    ...
    orderedGroups.forEach((gName) => {
      ...
      if (!gr) return;
      const slotsForThisGroup = Math.min(directCountByGroup[gName], remaining);
      for (let k = 0; k < slotsForThisGroup; k++) {
        const idx = extraPos - 1 + k;
        const candidate = gr.ranked[idx];
        // evita duplicar alguém que já classificou direto (pode acontecer se
        // "quantos melhores do geral" for grande o suficiente pra alcançar a
        // posição extra dentro da própria chave)
        if (candidate && !directIds.has(candidate.id)) {
          bonusCandidates.push({ ...candidate, groupName: gName, position: idx + 1 });
        }
      }
      remaining -= slotsForThisGroup;
    });
    extras = rankPlayers(bonusCandidates, sortBy);
  } else {
      ...
      >
        ...
        <div className="mb-4">
          ...
          <div className="grid sm:grid-cols-3 gap-2 max-w-2xl">
            ...
            >
              <p className="font-bold text-sm bt-text-ocean-deep">Melhores do geral + chave de origem</p>
              <p className="text-xs text-stone-500 mt-0.5">Os melhores do ranking geral classificam direto, e a(s) chave(s) de onde eles saíram também classifica(m) mais alguns, pelas posições extras configuradas abaixo.</p>
            </button>
          </div>
        </div>
        ...
        {classConfig.source === "geral" ? (
          ...
          <>
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-semibold bt-text-ocean-deep block mb-1.5">
                  {classConfig.source === "top2_terceiro" ? "Quantos melhores do geral classificam direto" : "Classificados direto por chave"}
                </label>
                <input
                  type="number" min={0}
                  className="bt-input w-full"
                ...
                <p className="text-xs text-stone-400 mt-1">
                  {classConfig.source === "top2_terceiro"
                    ? `Ex: 2 = os 2 melhores do ranking geral (não importa a chave) classificam direto.`
            ...
            <p className="text-xs text-stone-400 mt-2">
              {classConfig.source === "top2_terceiro"
                ? `Ex: posição extra = 3 e quantos melhores = 2 → a(s) chave(s) de onde saíram os classificados diretos também classificam o(s) 3º(s) colocado(s) dela(s). Se 2 diretos vierem da mesma chave, ela sozinha fornece o 3º e o 4º (posições sucessivas), pra sempre completar o total de "quantos melhores".`
